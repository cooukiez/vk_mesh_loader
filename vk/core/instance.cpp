...
void App::cursor_callback(GLFWwindow *window, const double nx, const double ny) {
    const auto app = static_cast<App *>(glfwGetWindowUserPointer(window));
    const glm::vec2 new_pos = {nx, ny};
    if (!app->cursor_enabled) {
        const glm::vec2 delta = new_pos - app->mouse_pos;
...
void key_callback(GLFWwindow* window, const int key, int scancode, const int action, int mods) {
    ...
    if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
        ...
        if (glfwGetWindowMonitor(window) == nullptr) {
            ...
            const int actual_width = mode->width / FULLSCREEN_RES_DIV;
            const int actual_height = mode->height / FULLSCREEN_RES_DIV;
...
void App::create_inst() {
    ...
    const auto exts = get_required_exts();
