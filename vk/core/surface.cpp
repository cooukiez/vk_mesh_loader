...
VkExtent2D App::choose_extent(const VkSurfaceCapabilitiesKHR &caps) const {
...
void App::create_swap() {
    const VCW_SwapSupport swap_support = query_swap_support(phy_dev);

    const VkSurfaceFormatKHR surf_format = choose_surf_format(swap_support.formats);
    const VkPresentModeKHR pres_mode = choose_pres_mode(swap_support.pres_modes);
    ...
    const uint32_t families[] = {qf_indices.qf_graph.value(), qf_indices.qf_pres.value()};
    ...
    for (const auto &base_img: swap_base_imgs) {
        VCW_Image img{};
        img.img = base_img;
...
void App::clean_up_swap() {
    ...
    for (const auto framebuffer: frame_bufs)
        vkDestroyFramebuffer(dev, framebuffer, nullptr);
    for (const auto img: swap_imgs)
